<template>
  <el-table stripe style="width: 100%; margin-bottom: 10px" v-bind="$attrs">
    ...
    <el-empty slot="empty" :image-size="80">
      <el-button slot="description" type="text" icon="el-icon-plus" :disabled="queryMode" @click="newItem">
        添加
      </el-button>
    </el-empty>
    ...
    <el-table-column fixed="right" align="center" width="50" min-width="10">
      ...
      <template slot="header" slot-scope="scope">
        <el-button type="text" icon="el-icon-plus" :disabled="queryMode" @click="newItem" />
      </template>
      <template #default="scope">
        <el-button type="text" icon="el-icon-delete" :disabled="queryMode" @click="removeItem(scope.$index)" />
      </template>
    </el-table-column>
  </el-table>
</template>
