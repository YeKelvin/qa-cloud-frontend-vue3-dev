  ...
  <div class="pymeter-component-container">
    ...
    >
      ...
      <el-form-item v-else-if="modifyMode">
        <el-button icon="el-icon-check" type="danger" @click="modifyAssertionElement">保 存</el-button>
        <el-button icon="el-icon-close" @click="closeTab">关 闭</el-button>
      </el-form-item>
      <el-form-item v-else-if="createMode">
        <el-button icon="el-icon-check" type="primary" @click="createAssertionElement">保 存</el-button>
        <el-button icon="el-icon-close" @click="closeTab">关 闭</el-button>
      </el-form-item>
    </el-form>
  </div>
...
export default {
  ...
  mounted: function () {
    ...
    if (this.queryMode || this.modifyMode) {
      ElementService.queryElementInfo({ elementNo: this.elementNo }).then((response) => {
        ...
        this.$refs.codeEditor.setValue(response.result.property.PythonAssertion__script)
      })
    }
  },
  ...
  methods: {
    ...
    async createAssertionElement() {
      ...
      // 关闭tab
      this.$store.commit('pymeter/removeTab', { editorNo: 'UNSAVED_PYTHON_ASSERTION' })
      // 重新查询脚本
      this.$store.commit('pymeter/refreshElementTreeNow')
    }
  }
