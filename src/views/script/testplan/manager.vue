<template>
  <div style="display:flex; flex-direction:column;">
        ...
        <el-table-column fixed="right" label="操作" min-width="150">
          <template slot-scope="{row}">
            <template v-if="row.state != 'COMPLETED'">
              <el-button type="text" style="margin-left:10px;" @click="gotoTestplan(row.planNo, 'QUERY')">计划详情</el-button>
              <el-button type="text" @click="openExecutionRecord(row)">执行记录</el-button>
              <el-button type="text" style="color:#F56C6C;font-weight:bold;" @click="executeTestplan(row)">立即执行</el-button>
            </template>
            <template v-else>
    ...
    <execution-record-dialog
      ...
      :visible.sync="showExecutionRecord"
  ...
  methods: {
    queryList() {
      TestplanService.queryTestplanList({
        ...
        workspaceNo: this.workspaceNo, page: this.page, pageSize: this.pageSize
      })
        .then(response => {
    ...
    gotoTestplan(planNo, editorMode) {
      this.$router.push({
        path: 'testplan/editor', query: planNo ? { planNo: planNo } : null,
        name: 'TestplanEditor', params: { editorMode: planNo ? editorMode : 'CREATE' }
      })
    },
