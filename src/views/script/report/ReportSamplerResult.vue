<template>
  <div class="details-container">
    ...
    <el-descriptions :column="1">
      ...
      <el-descriptions-item v-if="details.samplerRemark" label="请求描述">{{ details.samplerRemark }}</el-descriptions-item>
    </el-descriptions>

    <el-descriptions :column="3">
      <el-descriptions-item label="开始时间"><el-tag type="warning">{{ details.startTime }}</el-tag></el-descriptions-item>
      <el-descriptions-item label="结束时间"><el-tag type="warning">{{ details.endTime }}</el-tag></el-descriptions-item>
      <el-descriptions-item label="耗时"><el-tag type="danger">{{ details.elapsedTime }}</el-tag></el-descriptions-item>
    </el-descriptions>
    ...
    <el-tabs v-model="requestActiveTabName" @tab-click="handleRequestTabClick">
      ...
      <el-tab-pane key="HEADERS" label="请求头" name="HEADERS">
        <el-table :data="requestHeadersData" :show-header="false" size="mini" max-height="220" style="width: 100%">
      ...
      <el-tab-pane key="DATA" label="请求数据" name="DATA">
        <monaco-editor ref="requestEditor" key="requestEditor" :read-only="true" language="json" style="height:220px;" />
      </el-tab-pane>
    </el-tabs>
    ...
    <el-tabs v-model="responseActiveTabName" @tab-click="handleResponseTabClick">
      ...
      <el-tab-pane key="HEADERS" label="响应头" name="HEADERS">
        <el-table :data="responseHeadersData" :show-header="false" size="mini" max-height="220" style="width: 100%">
          <el-table-column prop="name" />
          <el-table-column prop="value" />
        </el-table>
      </el-tab-pane>
      ...
      <el-tab-pane key="DATA" label="响应数据" name="DATA">
        <monaco-editor ref="responseEditor" key="responseEditor" :read-only="true" language="json" style="height:220px;" />
      </el-tab-pane>
      <!-- 断言 -->
      <el-tab-pane v-if="details.failedAssertion" key="ASSERTION" label="断言" name="ASSERTION">
        <monaco-editor ref="assertionEditor" key="assertionEditor" :read-only="true" language="log" style="height:220px;" />
      </el-tab-pane>
    </el-tabs>
  </div>
</template>
