...
const totalData = computed(() => {
  return [
    {
      totalType: '总数',
      samplerTotal: details.value.successfulSamplersTotal + details.value.failedSamplersTotal
    },
    {
      totalType: '成功总数',
      samplerTotal: details.value.successfulSamplersTotal
    },
    {
      totalType: '失败总数',
      samplerTotal: details.value.failedSamplersTotal
    },
    {
      totalType: '成功率',
      samplerTotal: successRate(details.value.successfulSamplersTotal, details.value.failedSamplersTotal)
    },
    {
      totalType: '平均响应时间',
...
const queryGroupResult = () => {
  ReportService.queryGroupResult({ groupId: props.groupId }).then((response) => {
    details.value = response.result
    setPieChartData([
      { value: details.value.successfulSamplersTotal, name: '成功', itemStyle: { color: '#44B197' } },
      { value: details.value.failedSamplersTotal, name: '失败', itemStyle: { color: '#FF7676' } }
    ])
  })
}
