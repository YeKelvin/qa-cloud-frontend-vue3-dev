        ...
        <el-table-column fixed="right" label="操作" min-width="160" width="160">
          <template #default="{ row }">
            ...
            <template v-if="row.state === 'ENABLE'">
              <el-button type="primary" link @click="modifyPermissionState(row, 'DISABLE')">禁用</el-button>
            </template>
            <template v-else>
              <el-button type="primary" link @click="modifyPermissionState(row, 'ENABLE')">启用</el-button>
            </template>
            <el-button type="primary" link @click="removePermission(row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-card>
